...
from comet.background_scraper.cinemeta_client import (
    CinemetaClient,
...
class CinemetaSchemaTests(unittest.IsolatedAsyncioTestCase):
    def test_catalog_page_requires_metas_and_tolerates_pagination_extensions(self):
    ...
    async def test_catalog_pagination_isolates_items_and_uses_raw_page_size(self):
        client = CinemetaClient(session=object())
    ...
    def test_series_episodes_normalize_positive_coordinates(self):
        ...
        for invalid in ({}, {"meta": []}, {"meta": {"videos": {}}}):
            with self.subTest(payload=invalid):
                with self.assertRaises(ValueError):
                    _extract_series_episodes(invalid)

    async def test_catalog_failure_is_not_a_silent_end_of_pagination(self):
        client = CinemetaClient(session=object())
        client._fetch_catalog_page = AsyncMock(
            side_effect=RuntimeError("broken catalog")
        )

        with self.assertRaisesRegex(RuntimeError, "broken catalog"):
            _ = [item async for item in client.fetch_all_from_category("movie", "top")]
    ...
    def test_discovery_metadata_uses_usable_secondary_fields(self):
        worker = BackgroundScraperWorker()
        self.assertEqual(
            worker._normalize_media_item(
                {
